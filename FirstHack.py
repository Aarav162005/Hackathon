...
@app.route('/')
def home():
    return render_template('FirstHack.html', hospitals=hospitals, doctors=doctors, treatments=treatments)
...
def search():
    ...
    # Search hospitals, doctors, and treatments
    search_results = {
        'hospitals': [h for h in hospitals if query in h['name'].lower() or query in h['specialties'].lower()],
        'doctors': [d for d in doctors if query in d['name'].lower() or query in d['specialty'].lower()],
        'treatments': [t for t in treatments if query in t['name'].lower()]
    }
    return render_template('FirstHack.html', query=query, search_results=search_results)
